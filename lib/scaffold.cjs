...
// Which Fidj an app points at follows the API it talks to. It used to be the
// constant https://fidj.ovh, so every app built against the sandbox API sent
// people to a different deployment with a different database, where their apps
// are not — and the sandbox console never recognised itself as Fidj, because
// recognising itself is comparing its own origin against this value.
//
// An API host named after its console (api.X serves X) says where that console
// is. One that is not says nothing, and then the default stands rather than a
// guess being invented.
function dashboardFor(api) {
  return api.hostname.startsWith("api.")
    ? `https://${api.hostname.slice(4)}`
    : "https://fidj.ovh";
}
...
function scaffold(destination, options) {
  ...
  pkg.name = name;
  // The badge says which Fidj the app is running, so the version comes from the
  // SDK the app will actually carry: the local build when one is supplied, the
  // published range otherwise. A date told nobody which API it could talk to.
  let releaseVersion = pkg.dependencies["@ofidj/node"].replace(/^[\^~]/, "");
  if (options.sdkPath) {
  ...
  const configuration = {
    ...
    // A mounted module with its own patch (Fidj's console): the badge names
    // it between the SDK and the API.
    moduleVersion: appModule ? options.moduleVersion || "" : "",
    moduleLabel: options.moduleLabel || "module",
    moduleMount: null,
